...
def export(objectslist, filename, argstring):
    ...
    if argstring:
        ...
        for arg in args:
            ...
            elif arg == '--use-part-name':
                USE_PART_NAME = True
            elif arg.startswith('--precision='):
                PRECISION = int(arg.split('=')[1])
            elif arg.startswith('--workpiece-length='):
                WORKPIECE_LENGTH = float(arg.split('=')[1])
            elif arg.startswith('--workpiece-width='):
